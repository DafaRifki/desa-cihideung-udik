    ...
    <div>
      <section className="bg-sawah-800 py-14 text-beras-50 sm:py-16">
        <div className="container-desa">
          ...
          <h1 className="mt-3 font-display text-4xl font-semibold sm:text-5xl">Galeri Desa</h1>
          <p className="mt-3 max-w-xl text-sawah-100">
            Momen kegiatan warga, keindahan alam, dan budaya Desa Sukamakmur dalam gambar.
          </p>
        </div>
      </section>
      ...
      <Section>
          ...
          {albums.map((a) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                album === a ? 'bg-sawah-700 text-beras-50' : 'border border-sawah-200 text-ink-soft hover:bg-sawah-50'
              }`}
            >
              {a}
            </button>
        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {items.map((item, i) => (
              <GalleryItem key={item.id} item={item} onClick={() => setActiveIndex(i)} />
            ))}
          </div>
        )}
      </Section>
